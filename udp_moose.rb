...
class UDPMoose
  ...
  def get_responses(request_id, &block)
  ...
  def receive(&block)
    ...
    loop do
      self.lock_receive
      self.wait_receive if @receives.empty?
      self.unlock_receive
  ...
  def listener
    ...
    Thread.new do
      ...
      loop do
        ...
        msg, addr = @udp_socket.recvfrom(64000)  # thread blocked until new packet recieved
        hash_msg = JSON.parse(msg)
        
        # handshake request received
        if @ingress && hash_msg["type"] == "stu"
        
          @buffers[hash_msg["id"]] = UDPMooseBuffer.new(hash_msg["total"], addr[2], addr[1]) unless @buffers[hash_msg["id"]]
          self.jack(hash_msg["id"]) unless @buffers[hash_msg["id"]]&.packet > 0
        ...
        # body fragment received
          ...
          if hash_msg["packet"] + 1 == buffer&.join!(hash_msg["packet"], hash_msg["content"]) && !buffer.completed?
            self.response(hash_msg["id"])
          elsif buffer&.completed?
            self.ivan(hash_msg["id"])
            @receives << buffer.buffer if buffer.enqueue?
            self.continue_receive unless @receives.empty?
          end
        ...
        # handwave request from server received
          ...
          if request
            ...
            self.bye(hash_msg["id"], addr[2], addr[1])
  ...
  class UDPMooseRequest
    ...
    def initialize(id, body, server_ip_range, server_port, max_connection, max_timeout)
      ...
      @id = id
      @body_fragments = self.split_body(body)
      @connections = {}
    ...
    def dequeue
      ...
      @timestamp = Time.now.to_f if self.idle?
      connection
    ...
    def check_timeout
      ...
      if @connections[@id].connecting? && current_timestamp - @handshake_timestamp <= @max_timeout
        self.enqueue_by_connection_key(@id)
      ...
      if self.requesting? && current_timestamp - @timestamp > 1 && current_timestamp - @timestamp <= @max_timeout
  
        @connections.each do |key, connection|
          self.enqueue_by_connection_key(key) if !key == @id && @connection.connecting?
        end
  
      elsif self.requesting? && current_timestamp - @timestamp > @max_timeout
  
        @connections.each_value do |connection|
          connection.timeout if connection.connecting? # the closed connections won't be transferred to timeout connections, see UDPMooseConnection#close and UDPMooseConnection#timeout
        end
    ...
    def split_body(body, packet_limit = 60000)
  
      body_fragments = []
  
      i = 15000 # index of string character
      temp_string = body[0...15000]
  ...
  class UDPMooseHandShakeConnection < UDPMooseConnection
    ...
    def initialize(id, body_fragments, server_ip_range)
      super(id, body_fragments, server_ip_range)
    end
  ...
  class UDPMooseBuffer
    ...
    def join!(packet, content)
      ...
      if @packet == packet && !self.completed?
        @buffer += content
        @packet += 1
      end
